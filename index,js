[Reasoning]
The user wants the full index.js in one clean code block. Let me provide it.

Here's the complete `index.js` — copy everything below the line:
...
// ── Persistent token store (Upstash Redis) ───────────────────────────────────
...
function tmdbToMeta(item, type = 'movie') {
    const id = type === 'movie' ? `tmdb:${item.id}` : `tmdb:${item.id}`;
    return {
        id,
        type,
...
app.get('/meta/:type/:id.json', async (req, res) => {
    try {
        const { type, id } = req.params;
        setCacheHeaders(res);
        // basic meta passthrough — extend as needed
...
app.get('/auth/poll', async (req, res) => {
    try {
        ...
        if (data.access_token) {
            ...
            saveTokens(tokenStore);   // persist to Redis
        }
        res.json(data);
    } catch (e) {
        res.status(500).json({ error: e.message });
...
(async () => {
    tokenStore = await loadTokens();   // hydrate from Redis before serving
    app.listen(PORT, () => console.log(`🐍 Snakeeyes Simkl addon listening on ${PORT}`));
})();
```

Paste that into `index.js`, commit, and push — then set `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` in Render's environment variables.
