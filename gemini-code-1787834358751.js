// Snakeeyes Simkl Addon — Full Complete Version with All-Items Sync
...
async function getSimklCatalog(catalogId) {
    ...
    const map = {
        'simkl-history-movies':   { type: 'movies', status: 'completed', mediaType: 'movie' },
        'simkl-history-series':   { type: 'tv',     status: 'completed', mediaType: 'series' },
        'simkl-watchlist-movies': { type: 'movies', status: 'plantowatch', mediaType: 'movie' },
        'simkl-watchlist-series': { type: 'tv',     status: 'plantowatch', mediaType: 'series' },
        'simkl-ratings-movies':   { type: 'movies', status: 'rated',     mediaType: 'movie' },
        'simkl-ratings-series':   { type: 'tv',     status: 'rated',     mediaType: 'series' },
    };
    const def = map[catalogId];
    if (!def) return [];

    try {
        const data = await simklGet(`/sync/all-items/${def.type}/${def.status}`, accessToken);
        
        let list = [];
        if (Array.isArray(data)) {
            list = data;
        } else if (data) {
            list = data.movies || data.shows || data.anime || data.list || data.completed || data.plantowatch || data.rated || [];
            if (list.length === 0) {
        ...
        return list.map(item => {
            ...
            const ids = mediaObj.ids || item.ids || {};
            
            const imdbId = ids.imdb;
            const tmdbId = ids.tmdb;
            const id = imdbId ? `tt:${imdbId}` : (tmdbId ? `tmdb:${tmdbId}` : null);
            
            const title = mediaObj.title || item.title || 'Untitled';
