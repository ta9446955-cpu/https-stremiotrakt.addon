...
<head>
  ...
  <style>
    ...
    h1 { color: #ed1c24; }
    .btn { background: #ed1c24; color: white; padding: 14px 28px; border: none; border-radius: 6px; font-size: 1.1em; cursor: pointer; display: inline-block; margin: 10px; }
...
app.get('/auth/start', async (req, res) => {
  if (!TRAKT_CLIENT_ID) return res.json({ error: 'TRAKT_CLIENT_ID not set' });
  try {
    const response = await fetch('https://api.trakt.tv/oauth/device/code', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'User-Agent': 'SnakeyesTraktAddon/1.0' },
      body: JSON.stringify({ client_id: TRAKT_CLIENT_ID.trim() })
    });
    const text = await response.text();
    if (!response.ok) return res.json({ error: `Trakt returned ${response.status}` });
    const data = JSON.parse(text);
    res.json({ device_code: data.device_code, user_code: data.user_code, expires_in: data.expires_in });
...
app.get('/auth/poll', async (req, res) => {
  ...
  try {
    const response = await fetch('https://api.trakt.tv/oauth/device/token', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'User-Agent': 'SnakeyesTraktAddon/1.0' },
      body: JSON.stringify({ code: deviceCode, client_id: TRAKT_CLIENT_ID.trim(), client_secret: TRAKT_CLIENT_SECRET.trim() })
    });
    if (response.status === 200) {
      const tokens = await response.json();
      const userKey = crypto.randomBytes(8).toString('hex');
      tokenStore[userKey] = { access_token: tokens.access_token, refresh_token: tokens.refresh_token };
      return res.json({ ready: true, key: userKey });
    } else if (response.status === 400) {
      return res.json({ ready: false });
    } else {
      const errText = await response.text();
      return res.json({ ready: false, error: `Trakt ${response.status}: ${errText}` });
    }
  } catch (e) { return res.json({ ready: false, error: e.message }); }
});
...
app.get('/:userKey/catalog/:type/:catalogId.json', async (req, res) => {
  ...
  try {
    let items = [];
    if (catalogId.includes('history')) items = await traktGet(`/users/me/history/${typeParam}?limit=50`, userData.access_token);
    else if (catalogId.includes('watchlist')) items = await traktGet(`/users/me/watchlist/${typeParam}`, userData.access_token);
    else if (catalogId.includes('ratings')) items = await traktGet(`/users/me/ratings/${typeParam}`, userData.access_token);
    else if (catalogId.includes('collection')) items = await traktGet(`/users/me/collection/${typeParam}`, userData.access_token);
    const metas = items.map(item => traktItemToMeta(item, type)).filter(Boolean)
...
app.get('/:userKey/meta/:type/:id.json', async (req, res) => {
  ...
  if (!tokenStore[userKey]) return res.json({ meta: {} });
  res.json({ meta: { id, type } });
});

app.get('/health', (req, res) => {
  res.json({ status: 'ok', users: Object.keys(tokenStore).length });
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`Snakeeyes Trakt Addon running on port ${PORT}`));
