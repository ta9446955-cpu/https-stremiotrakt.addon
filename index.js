...
async function fetchTMDBPoster(imdbId, type) {
  if (!TMDB_API_KEY) return { poster: null, background: null };
  try {
    const res = await fetch(`https://api.themoviedb.org/3/find/${imdbId}?api_key=${TMDB_API_KEY}&external_source=imdb_id`);
    if (!res.ok) return { poster: null, background: null };
    const data = await res.json();
    const results = type === 'movie' ? data.movie_results : data.tv_results;
    if (!results || results.length === 0) return { poster: null, background: null };
