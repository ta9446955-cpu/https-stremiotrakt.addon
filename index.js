...
// ── Landing page ─────────────────────────────────────────────────────────────
app.get('/', (req, res) => {
    setCacheHeaders(res, 60); // Short cache for landing page
...
app.get('/:userKey/manifest.json', (req, res) => {
    ...
    setCacheHeaders(res, 60 * 60 * 24 * 30); // Cache manifest for 30 days
...
app.get('/:userKey/catalog/:type/:catalogId.json', async (req, res) => {
    ...
    // Set long cache for all catalog responses
    ...
    // Simkl personal catalog - these should not be cached as long since they're user-specific
    // But we'll still cache them for a few hours
    setCacheHeaders(res, 60 * 60 * 4); // 4 hours for personal data
