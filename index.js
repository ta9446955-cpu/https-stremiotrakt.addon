...
// Fetch external IDs (like IMDb tt...) for TMDB items so streaming addons match them
async function enrichWithImdbId(tmdbId, type) {
    try {
        ...
        if (data && data.imdb_id) {
            return data.imdb_id; // Returns e.g. "tt0111161"
        }
    } catch (e) {}
    return `tmdb:${tmdbId}`;
}
...
async function getTmdbCatalog(catalogId, type) {
    ...
    // Process items in parallel to fetch IMDb IDs
