// Snakeeyes Simkl Addon — Fixed version for Stremio
...
console.log('🚀 Starting Snakeeyes Simkl Addon...');
console.log('TMDB_API_KEY set:', !!TMDB_API_KEY);
console.log('SIMKL_CLIENT_ID set:', !!SIMKL_CLIENT_ID);
...
// ── TMDB catalog map - CORRECTED COLLECTION IDs ────────────────────────────
const tmdbCatalogMap = {
    ...
    // Franchises - VERIFIED COLLECTION IDs
    'tmdb-starwars':         { endpoint: 'collection', params: '10' },          // Star Wars
    'tmdb-marvel':           { endpoint: 'collection', params: '86311' },       // Marvel Cinematic Universe
    'tmdb-dc':               { endpoint: 'collection', params: '8537' },        // DC Extended Universe
    'tmdb-lotr':             { endpoint: 'collection', params: '119' },         // Lord of the Rings
    'tmdb-harrypotter':      { endpoint: 'collection', params: '1241' },        // Harry Potter
    'tmdb-jurassicpark':     { endpoint: 'collection', params: '328' },         // Jurassic Park
    // Directors
...
// ── TMDB API helper - Fixed to handle all endpoints properly ──────────────
async function tmdbFetch(endpoint, params, type) {
    if (!TMDB_API_KEY) {
    ...
    try {
        ...
        if (endpoint === 'person' && params.includes('/movie_credits')) {
...
app.get('/:userKey/catalog/:type/:catalogId.json', async (req, res) => {
    ...
    if (tmdbCatalogMap[catalogId]) {
        try {
            ...
            const metas = await tmdbFetch(endpoint, params, type);
