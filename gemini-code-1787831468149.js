...
// Updated with IMDb ID resolution for seamless streaming links
...
app.get('/configure', (req, res) => {
    res.send(`
        ...
        <head>
            ...
            <style>
                body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; background: #121212; color: #fff; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; }
                .card { background: #1e1e1e; padding: 30px; border-radius: 12px; box-shadow: 0 8px 24px rgba(0,0,0,0.5); text-align: center; max-width: 450px; width: 100%; box-sizing: border-box; }
                ...
                #status { margin-top: 15px; font-size: 14px; color: #46d369; word-break: break-all; }
                input { width: 100%; padding: 10px; margin-top: 10px; background: #2a2a2a; border: 1px solid #444; color: #fff; border-radius: 4px; box-sizing: border-box; text-align: center; }
            </style>
        </head>
        <body>
            <div class="card">
                    ...
                    const interval = setInterval(async () => {
                        try {
                            ...
                            if (data.success) {
                                ...
                                const manifestUrl = window.location.origin + '/' + userKey + '/manifest.json';
                                status.style.color = '#46d369';
                                status.innerHTML = '<b>Successfully authenticated!</b><br>Copy your Manifest URL below and paste it into Stremio:<br><input type="text" value="' + manifestUrl + '" readonly onclick="this.select()">';
                            }
                        } catch (e) {}
                    }, 5000);
                }
