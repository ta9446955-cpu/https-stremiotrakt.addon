// Snakeeyes Simkl Addon — Full Complete Version with Aggregated Sync Parser
...
async function getSimklCatalog(catalogId) {
    const tokenObj = tokenStore['default'] || Object.values(tokenStore)[0];
    if (!tokenObj || !tokenObj.access_token) return [];
    const accessToken = tokenObj.access_token;

    const isSeries = catalogId.includes('series');
    const simklType = isSeries ? 'shows' : 'movies';

    try {
        const data = await simklGet(`/sync/all-items/${simklType}`, accessToken);
        
        let list = [];
        if (Array.isArray(data)) {
            list = data;
        } else if (data && typeof data === 'object') {
            const possibleKeys = [simklType, 'movies', 'shows', 'anime', 'completed', 'plantowatch', 'rated', 'watching', 'hold', 'dropped', 'list'];
            for (const k of possibleKeys) {
                if (Array.isArray(data[k])) {
                    list = list.concat(data[k]);
                }
            }
            if (list.length === 0) {
